...
| 目前階段 | S1 / C1 / I1a / I1b-A / J7d-A / 忘記密碼 / M1a（A+B+C）/ admin 刪除草稿 / M1b 第一批 6 個 JYP / trust_level 單一真相 / Supabase SSR middleware / 後台 4 頁 filter tabs + 搜尋 pilot（events、idols、event-candidates、sources）完成。下一步建議：M1b 第二批（非 JYP via kpopofficial 聚合 + alt_names）、補齊新 JYP 藝人 UI、或 J7d-B（累積實況後）|
...
| 44 | M1b 第二批 | 非 JYP 藝人（IVE / aespa / NewJeans / BTS / SEVENTEEN / ATEEZ …）：補 idols + alt_names，讓 kpopofficial 聚合來源自動撿活動；不必每團做新 crawler | 🔲 待辦（建議優先，純 migration）|
| 45 | 補新藝人 UI | 6 個 M1b 第一批藝人需要補 avatar / 描述 / 主色（/admin/idols/[id]/edit 手動）| 🔲 待辦（小工，不急）|
| 46 | M2 | 跨來源活動去重（event_key soft-hash，同演唱會多來源合併）| 🔲 待辦（M1b 第二批後）|
| 47 | 帳號設定 | 改 email、刪除帳號、2FA | 🔲 待辦 |
| 48 | M3 | Custom SMTP / Resend（上線前避免 rate limit）| 🔲 待辦 |
| 49 | Apple Sign-In | 上 App Store 前再做 | 🔲 待辦 |
| 50 | I1b-B | AI 搜圖（Claude web_search / Bing）+ 候選縮圖選擇 + 自動下載並上傳到 Storage + 寫回 avatar_url | 🔲 待設計討論（之前列為 🔒 工作單，新工作流改為 session 內討論定案）|
...
│       ├── idols/
│       │   ├── page.tsx + IdolsClient.tsx      # filter tabs + 搜尋（PR #53）
│       │   ├── [id]/page.tsx + actions.ts
