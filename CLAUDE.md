...
| 目前階段 | S1 / C1 / I1a / I1b-A / J7d-A / 忘記密碼 / M1a / admin 刪除草稿 / M1b 第一批 + 第二批（51 個主流 K-pop 藝人）/ trust_level 單一真相 / Supabase SSR middleware / 後台 4 頁 filter tabs + 搜尋 pilot / 後台 idols「缺資料」tab / I1b-B AI 搜圖（Wikimedia + 可重搜）/ I1b-C avatar 來源紀錄 + 多色 placeholder / 前台 polish 6 改動（filter 軸 / 查看全部 / 連結待補 / 收藏搜尋 / chip scroll / 偶像詳情頁）/ #45 description + color 補齊 / Source Inventory A 官方來源盤點 完成。下一步建議：M1b-3 migration 038（依 Source Inventory A 新增 solo / 分隊 / 大團成員）→ crawler 工作單（YG / WAKEONE / SM / Weverse 技術探測） |
...
**來源盤點注意：** `SOURCE_INVENTORY_A.md` 是 PR #70 產出的研究附件，用來支援 M1b-3 seed 與 crawler 工作單。它不是新的工作紀錄中心；session 交接、進度索引、下一步仍以本 `CLAUDE.md` 為唯一入口。
...
| 52 | M1b-3 | migration 038：依 Source Inventory A 新增 solo / 獨立藝人 / 分隊 / 大團成員，並修 aespa agency typo（`SSM Entertainment` → `SM Entertainment`）。候選含 Rain、IU、TAEYEON、G-DRAGON、BLACKPINK solo、BTS solo、NCT 分隊等；seed 前須依來源盤點確認 agency / official source | 🔲 待辦（migration 038，只給 SQL 等使用者手動跑）|
