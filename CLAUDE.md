...
本專案可以使用 RTK 來減少 terminal output token 消耗，特別是以下常見指令：
...
RTK 的用途是壓縮與整理終端機輸出，幫助節省 token。但 RTK 不應該取代完整錯誤判斷。

以下高風險情況，**必須查看完整原始輸出**，或重新執行不經 RTK 壓縮的指令後再判斷：

- Supabase migration 錯誤
- RLS / 權限錯誤
- Auth / OAuth / callback 問題
- Payment / webhook 問題
- Database schema 變更
- 資料寫入 / 刪除 / migration 相關問題
- `npm run build` 出現不明原因錯誤
- 測試大量失敗
- 任何可能造成資料遺失、登入失效、付款錯誤或權限漏洞的問題

**原則：**

1. 一般 build / test / git 指令可使用 RTK。
2. 如果錯誤原因不清楚，請主動 rerun 並查看完整 raw output。
3. 不要只根據 RTK 摘要做高風險工程決策。
4. 回報時請說明是否有使用 RTK，以及高風險錯誤是否已查看完整原始輸出。
