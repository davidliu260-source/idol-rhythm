# N7 工作單：followed_idol_new_event 通知派送

> **狀態：待 GPT review**
> **關聯進度：** WORKING.md Item 85 N7
> **前置條件：** migration 042 已執行 ✅、N6 cron 實作（PR #119 merged）✅
...
- N7：admin 發布活動 → 比對追蹤名單 → 立即派送（或每日批次）
...
- `dedupe_key = followed_idol_new_event:{event_id}:{user_id}` 防重複
...
- 回追舊活動（backfill：`user_follows.created_at` 之前的活動不補送）
- 自動發布 / 自動 approve
- 通知設定 toggle（未來 N8）
