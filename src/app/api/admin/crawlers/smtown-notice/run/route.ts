...
import { runSmtownNoticeFetcher } from '@/lib/crawlers/runSmtownNoticeFetcher'
 ...
 *
 * Admin-only manual trigger for the SMTOWN notice fetcher.
 * Body: { sourceKey: string }  (parser_type must be 'smtown_notice')
 *
 * Valid sourceKey values (after migration 045 is executed):
 *   - aespa-smtown-notice
 *   - riize-smtown-notice
...
export async function POST(
    ...
    return buildResponse(
      {
        ...
        skipped: 0,
        recheck: 0,
        errors: ['請在 body 提供 sourceKey（例如 aespa-smtown-notice）'],
      },
      400,
    )
  }
  ...
  const result = await runSmtownNoticeFetcher(supabase, { sourceKey })
