...
> 建立日期：2026-05-15
> 最後更新：2026-05-17（J0–J7a 完成；jyp_schedule 平台化 + Stray Kids 種子上線）
...
### ✅ Phase H3：Edit Idol Basic Info（已完成）

**路由**：`/admin/idols/[id]/edit`

**已實作**：
- `supabase/migrations/010_admin_users_update_idols_basic_policy.sql`：column-level `GRANT UPDATE` on content fields（slug、is_active、id、created_at 排除）+ UPDATE policy（admin_users-based）
- Server Action `updateIdol(idolId, payload)`：admin guard + UPDATE + revalidatePath（/admin/idols、/admin/idols/[id]、/idols）+ redirect
- `EditIdolForm.tsx`：初始值由 server props 傳入，slug 顯示但 disabled，is_active 不在表單（H4）
- `edit/page.tsx`：admin guard + idol 不存在 guard + 預填 initial 值

**詳情頁更新**：
- `/admin/idols/[id]/page.tsx` 加入 `getCurrentAdmin()`（Promise.all）
- admin 看到「編輯偶像資料」連結；非 admin 維持只讀 banner

**Commit**：`7cc1f12`  
**Migration 010**：✅ 已執行  
**人工驗收**：✅ 通過（2026-05-15，aespa 編輯表單預填正確）

---

### ✅ Phase H4：Toggle Idol is_active（已完成）

**路由**：`/admin/idols/[id]`（按鈕操作）

**已實作**：
- `supabase/migrations/011_admin_users_toggle_idol_active_policy.sql`：`GRANT UPDATE (is_active) ON idols` + UPDATE policy
- 詳情頁「停用偶像 / 啟用偶像」按鈕，一鍵切換 `is_active`
- `activateIdol` / `deactivateIdol` server actions
- 停用後不出現在前台偶像頁 + 不出現在新增活動的偶像下拉選單

---

### ✅ Phase I：event_candidates Review（已完成）

**路由**：`/admin/event-candidates`、`/admin/event-candidates/[id]`

**已實作**：
- `supabase/migrations/012_admin_users_review_event_candidates_policy.sql`：GRANT SELECT + GRANT UPDATE(review_status, reviewer_note, approved_event_id) + SELECT/UPDATE policies
- 候選列表：pending 優先排序 + 三組統計（待審核 / 已核准 / 已拒絕）
- 候選詳情：Approve（建立草稿 event，trust_level=pending）+ Reject 按鈕 + reviewer_note
- `approveCandidate` / `rejectCandidate` server actions

---

### ✅ Phase J（J0–J4）：AI / Crawler Pipeline（已完成）

**J0：設計文件**（`cf1bd6a`）
- `AI_PIPELINE_PLAN.md`：6 階段 pipeline 設計、資料流盤點、安全邊界

**J1：手動匯入候選**（`4c1baf0`）
- `/admin/event-candidates/new`：表單頁 + `createCandidate` server action
- `source_hash` SHA-256 去重；`raw_data: { source: 'manual' }`

**J2：BLACKPINK 官方 tour fetcher**（`acf7952`）
- `src/lib/crawlers/blackpinkOfficialTour.ts`：cheerio HTML parser
- `src/app/api/admin/crawlers/blackpink-tour/run/route.ts`：POST 手動觸發（admin guard + 雙 `.in()` 去重）
- `src/app/admin/event-candidates/CrawlerButton.tsx`：Client button，顯示中文結果摘要

**J4：source_hash 去重強化**（`2c2ec1c`）
- `src/lib/crawlers/sourceHash.ts`：SHA-256（URL 優先 / 欄位 fallback）
- `supabase/migrations/016_admin_users_insert_event_candidates_policy.sql`：INSERT policy ✅ 已執行
- `supabase/migrations/017_event_candidates_dedupe_fields.sql`：source_hash + raw_data columns + unique index ✅ 已執行
- 23505 unique_violation → 中文友善提示

**J3：AI 解析公告**（`3cd09f2`）
- `src/lib/ai/parseCandidate.ts`：Claude Haiku wrapper，resolveAnthropicModel()，JSON fence 提取，slug → UUID 解析
- `src/app/api/admin/ai/parse-candidate/route.ts`：POST API（admin guard + known_idols 查詢）
- `/admin/event-candidates/parse`：Server Component（admin guard）+ `ParseClient.tsx`（預覽卡 + 信心顏色）+ `commitAiCandidate` server action
- `ANTHROPIC_API_KEY`：本地 `.env.local` + Vercel Production/Preview

---

### ✅ Phase J5：Cron 自動觸發（完成）

**J5**（`4c911e7`）：`vercel.json` `"0 1 * * *"`（09:00 Asia/Taipei），`CRON_SECRET` header 驗證，`?dryRun=1` 乾跑保留。

**J5b**（`19ed919`）：`src/lib/supabase/serviceClient.ts`（`import 'server-only'`）；migration 018（`GRANT SELECT / INSERT / UPDATE ON event_candidates TO service_role`）✅ 已執行；Production 驗收通過。

---

### ✅ Phase J6：多來源 fetcher 擴充（完成）

**J6a**（`3490082`，migration 019）：`crawler_sources` table + `/admin/sources` 列表 + 詳情頁 + crawler helpers。

**J6b**（`811a473`，migration 020）：crawler_sources RLS + BLACKPINK fetcher 改由 DB 行驅動 + 通用 `RunSourceButton`。

**J6c→J6d**（`452d995`，migration 022，PR #13）：JYP JSON API 整合；通用 `jyp_schedule` parser；`config jsonb` 欄位；零程式碼加新 JYP 藝人。

**J6e**（`4449cc7`，PR #14）：Cron fan-out — 列出所有 `is_active=true` sources，依 `parser_type` dispatch（sequential，獨立 try/catch）。

**J6f**（`46f197f`，PR #15）：`MONTHS_AHEAD=12`，`MAX_ENTRIES_PER_RUN=200`，過去日期過濾（`detectedDate >= today`）。

---

### ✅ Phase J7a：Stray Kids 種子（完成，`e058de0`，PR #16）

migration 023：INSERT idols（stray-kids）+ INSERT crawler_sources（stray-kids-jyp-schedule, config={groupId:'10'}）；零程式碼，驗證 J6d 設計契約。**待執行**：migration 023 需人工在 Supabase SQL Editor 執行；PR #16 待 merge。

---

### 🔒 Phase J7b：批量審核 UI（待實作）

checkbox + 全選 + 批量 Approve / Reject 工具列；`POST /api/admin/event-candidates/bulk-review`。

---

### 🔒 Phase J7c：過期候選清理（待實作）

過濾 `pending` + `detected_date < today`，批量設 `review_status='rejected'`（不 DELETE，保留記錄）。

---

### 🔒 Phase J7d：內容變更偵測（待實作）

`content_hash text` 欄位 + `needs_recheck` flag；fetcher source_hash 命中但 content_hash 不同時標記；Admin UI badge。需獨立 migration 工作單。

---

### 🔒 Phase K：Analytics Dashboard（待實作）
...
| `/admin/events/[id]` | ✅ 完成 | 活動詳情 + 發布 / 下架按鈕 |
| `/admin/events/new` | ✅ 完成 | 草稿新增閉環，人工驗收通過 |
| `/admin/events/[id]/edit` | ✅ 完成 | 草稿編輯，人工驗收通過 |
| `/admin/idols` | ✅ 完成 | Phase H1（偶像列表，管理員可見新增按鈕） |
| `/admin/idols/[id]` | ✅ 完成 | Phase H2/H4（只讀詳情 + 啟用/停用按鈕） |
| `/admin/idols/new` | ✅ 完成 | Phase H2（新增偶像表單，admin guard） |
| `/admin/idols/[id]/edit` | ✅ 完成 | Phase H3（偶像基本資料編輯，slug 不可改） |
| `/admin/event-candidates` | ✅ 完成 | Phase I + J1/J2/J3（列表 + AI解析 + CrawlerButton + 新增候選） |
| `/admin/event-candidates/[id]` | ✅ 完成 | Phase I（Approve / Reject + source_hash 顯示） |
| `/admin/event-candidates/new` | ✅ 完成 | J1 手動匯入候選表單 |
| `/admin/event-candidates/parse` | ✅ 完成 | J3 AI 解析公告（Claude Haiku） |
| `/admin/sources` | ✅ 完成 | J6a 爬蟲來源列表（parser_type / is_active / 上次執行） |
| `/admin/sources/[id]` | ✅ 完成 | J6b 爬蟲來源詳情 + 通用手動觸發按鈕（RunSourceButton） |
| `/admin/analytics` | 🔒 未實作 | Phase K |
...
| Phase H3：Edit Idol Info | 🟡 中 | ✅ 完成（`7cc1f12`），migration 010 已執行 |
| Phase H4：Toggle is_active | 🔴 高 | ✅ 完成（migration 011） |
| Phase I：Candidates Review | 🔴 高 | ✅ 完成（migration 012） |
| J0：AI Pipeline 設計 | 🟢 低 | ✅ 完成（`cf1bd6a`） |
| J1：手動匯入候選 | 🟡 中 | ✅ 完成（`4c1baf0`，migration 016） |
| J2：BLACKPINK fetcher | 🟡 中 | ✅ 完成（`acf7952`） |
| J4：source_hash 去重 | 🔴 高 | ✅ 完成（`2c2ec1c`，migration 017） |
| J3：AI 解析（Claude Haiku） | 🔴 高 | ✅ 完成（`3cd09f2`，ANTHROPIC_API_KEY） |
| J5：Cron 自動觸發 | 🔴 高 | ✅ 完成（`4c911e7`，migration 018） |
| J5b：Cron 安全寫入 | 🔴 高 | ✅ 完成（`19ed919`，Production 驗收通過） |
| J6a：crawler_sources table | 🟡 中 | ✅ 完成（`3490082`，migration 019） |
| J6b：crawler_sources RLS + admin UI | 🟡 中 | ✅ 完成（`811a473`，migration 020） |
| J6c→J6d：JYP platform fetcher | 🟡 中 | ✅ 完成（`452d995`，migration 021/022，PR #13） |
| J6e：Cron fan-out | 🟡 中 | ✅ 完成（`4449cc7`，PR #14） |
| J6f：12-month window + past filter | 🟢 低 | ✅ 完成（`46f197f`，PR #15） |
| J7a：Stray Kids 種子 | 🟢 低 | ✅ 完成（`e058de0`，migration 023，PR #16 open）|
| J7b：批量審核 UI | 🟡 中 | 🔒 待實作 |
| J7c：過期候選清理 | 🟢 低 | 🔒 待實作 |
| J7d：內容變更偵測 | 🟡 中 | 🔒 待實作（需 migration） |
| Phase K：Analytics | 🟡 中 | 🔒 待前台 Auth + 資料累積 |
...
✅ Phase H3（Idol Edit — `7cc1f12`，migration 010 已執行，人工驗收通過）
✅ Phase H4（Toggle is_active — migration 011 已執行）
✅ Phase I（Candidates Review — migration 012 已執行）
✅ Auth Milestone 1（Email magic link + saved_events — migration 013）
✅ Auth Milestone 2（Google OAuth）
✅ Auth Milestone 3（Email + Password 登入 / 註冊）
✅ Auth Milestone 4（user_follows 持久化 — migration 015）
✅ J0（AI_PIPELINE_PLAN.md — `cf1bd6a`）
✅ J1（手動匯入候選 — `4c1baf0`，migration 016）
✅ J2（BLACKPINK fetcher — `acf7952`）
✅ J4（source_hash 去重 — `2c2ec1c`，migration 017）
✅ J3（AI 解析公告 — `3cd09f2`，ANTHROPIC_API_KEY 已設定）
✅ J5（Cron 自動觸發 — `4c911e7`，vercel.json，CRON_SECRET）
✅ J5b（Cron 安全寫入 — `19ed919`，serviceClient.ts，migration 018，Production 驗收通過）
✅ J6a（crawler_sources table — `3490082`，migration 019）
✅ J6b（crawler_sources RLS + admin UI — `811a473`，migration 020）
✅ J6c→J6d（JYP platform fetcher — `452d995`，migration 021/022，PR #13 merged）
✅ J6e（Cron fan-out — `4449cc7`，PR #14 merged）
✅ J6f（12-month window + past filter — `46f197f`，PR #15 merged）
✅ J7a（Stray Kids 種子 — `e058de0`，migration 023，PR #16 open）
    ↓
[下一步：J7b 批量審核 UI → J7c 過期候選清理 → J7d 內容變更偵測]
    ↓
Phase K（Analytics）
```
