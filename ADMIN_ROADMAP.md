...
> 建立日期：2026-05-15
> 最後更新：2026-05-15（Phase F 發布 / 下架流程程式碼完成，待 migration 006 人工執行）
...
### ⏳ Migration 006（程式碼完成，待人工執行）

| Migration | 說明 | Commit |
|---|---|---|
| `006_admin_users_publish_events_policy.sql` | events column-level GRANT UPDATE + UPDATE RLS policy（發布 / 下架用）| `cf4049a` |
...
⏳ Migration 006（待人工在 Supabase SQL Editor 執行 + 人工驗收 Phase F）
    ↓
[下一個候選：Phase G — 草稿編輯（需獨立工作單 + UPDATE policy 範圍確認）]
    ↓
Phase G（Event Edit）
    ↓
