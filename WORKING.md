...
| 項目 | 值 |
|---|---|
| 目前階段 | **2026-07-15 Checkpoint（聚合源自動求證：B-0 Gemini 否決 → B-0c Claude 通過 → B-1 spike 完成：citation binding 已解 → 下一步 B-1a negative control）**：**流程已改**（見 CLAUDE.md 角色分工）：PM(Claude) 開票+附可複製指令 → Owner 貼給 Codex → Codex 實作 → PM review PR → Owner merge。**已廢除 GPT audit**。⚠️ **Codex 反覆回報「gh token 失效」，但 PM 端實測三次皆正常**（`gh api user` 回 davidliu260-source）→ Codex 卡 PR 時直接由 PM 開，Owner 不需登入 GitHub。**B-0（Gemini）Verdict BLOCKED**（PR #187 **merged 2026-07-15**）：2.5→404、3.x→Search grounding quota 0/429，0/5 完成，provider 指標不可量測；需開 Google billing（**預付 $10、12個月到期作廢、要填台灣統編**）才能重跑 → **Owner 決定不走此路，Gemini key 雖已存 .env.local 但用不到**。**B-0c（Claude web search）Verdict 需調整**（PR #188 **merged 2026-07-15**）：同一批 5 筆、同一判定標準 → **命中 5/5、假陽性 0/5（通過 Owner 指定的最關鍵 gate）**、provider error 0/5；14 searches / 225,634 in / 7,258 out = **US$0.925772（$0.185/event）**；用**現有 `ANTHROPIC_API_KEY`，零新 billing**。**但 structured citations 0/5**（text blocks 無 citations array、cited_text 全缺，模型僅以純文字 SOURCE_URLS 選證據 → regex 解析散文過脆弱，不可靠落庫）→ 故 Verdict 為「需調整」而非通過。**成本推估**：94 筆卡住草稿全查 ≈ **$17（優化後 ≈$11）**，錢非考量；費用主體是 search result 進 input context 的 token，不是 $10/1k search 本身 → 省錢槓桿是 `max_uses`。**B-1 spike 完成（2026-07-15，報告 `AGGREGATOR_VERIFICATION_B1_SPIKE_REPORT.md`）：假設成立，citation binding 已解**。同一批 5 筆跑 3 組共 15 requests：**A**（`web_search_20260209` 預設 dynamic filtering）= citations **0/5**；**B**（同 tool + `allowed_callers:["direct"]`）= citations **5/5**、cited_text 5/5；**C**（`web_search_20250305` basic）= citations **5/5**。A/B 唯一變因是 `allowed_callers`，model / candidate / prompt / max_uses / blocked domain 全同 → **dynamic filtering 的 code-execution caller 是 citation binding 斷裂主因，結論可信**。**B-direct = runtime contract 首選**：$0.049124/event、latency 9.508s、每筆僅 1 search，比 B-0c 省 **73.5% 成本 / 79.6% latency**；C-basic 列 fallback（僅在 Anthropic 未來移除 direct caller 時才切）。**PM 已獨立驗算報告全部成本 / token / latency / 百分比 → 數字全對，無估算冒充實測**。⚠️ **但 PM review 發現一個空指標，必須知道**：三輪（B-0/B-0c/B-1）都用**同一批 5 筆真實存在的大型巡演**（BTS×2 / ENHYPEN×2 / i-dle），正確答案本來就全是 CONFIRMED → **「假陽性 0/5」在結構上不可能失敗**（一個永遠回 CONFIRMED 的模型也會拿滿分），等於這個 gate 三輪都沒測到任何東西；contract §4.3 deterministic re-check / §4.4 fail closed **從未被考驗過**，而那 94 筆草稿裡必有日期錯 / 傳聞 / 張冠李戴的。**⚠️ 下一步 = B-1a negative control（票已開，Codex 已執行、報告待補，擋在 B-1b 前面）**：3 筆真候選 + 5 筆偽造（N1 日期偏移 / N2 場館置換 / N3 藝人置換 / N4 憑空捏造 / N5 日期集合不全），**偽造品只能在腳本記憶體中構造，嚴禁 INSERT 進 event_candidates 或任何 table**（DB 全程唯讀）；**不得為求好看改 prompt**（須與 B-1 B 組逐字相同，否則非對照實驗）；設定完全比照 B-direct；8 個獨立 request ≈ $0.4。**驗收 = 5/5 偽造被擋下（模型層或 deterministic 層任一）且 3/3 真候選 CONFIRMED**；任一偽造穿過兩層 → contract 需修正。**B-1a 未過不得進 B-1b migration**（先蓋房再驗地基＝可能白做整個 schema）。⚠️ **B-1 報告已知瑕疵**：§2.3 的 Los Angeles audit 註記寫「第三個 BIGHIT tour URL」，但 §2.5 完整列出的 3 個 citation 是 SoFi / SoFi / Live Nation，**無 BIGHIT** → 兩處必有一錯，引用 §2.3 註記需查原始 payload。**證據會蒸發**：B-0c / B-1 的 raw JSON 只在 `/tmp`，一清即無法複查，而它們正是 migration 的決策依據 → **B-1a 起要求 raw JSON 去除 `encrypted_content` 後提交進 repo**。**另注意**：spike 票的 `npm run build 通過` **不構成品質訊號**（該 commit 只新增 `.md`，build 必過），驗收看報告判定不看 build。🔥 **重大方向修正（2026-07-15，`BLOCKED_SITE_BROWSER_PROBE_REPORT.md`）：「封鎖清單」的框架是錯的**。PM 用真瀏覽器（App 內建 + Owner 的 Chrome）實測 4 個「封鎖 / 不要重複嘗試」站，結論是**進得去也沒用——經紀公司官網自己就沒有這些資料**：(G)I-DLE 日本官網 `gidle.cubeent.jp/schedule/` **零阻擋、完整渲染，但最新一筆是 2025.04.05（15 個月未更新）**，而 (G)I-DLE 確實有 2026 行程（Kai Tak 6/27–28，B-1a 已驗證）；BTS `ibighit.com/bts/eng/schedule/` 用 Owner 的 Chrome **進得去**（內建瀏覽器被拒 → 真瀏覽器確可繞過部分反爬），但該 URL **被導向 `/ko/main`，schedule 頁已不存在**；THEBLACKLABEL 是 **「유지 관리 모드」維護模式**（兩個月前 503，現仍關閉），不是反爬；Weverse 是**登入牆**不是反爬。→ **63 個零候選黑洞的成因不是「爬不到官網」，是「官網沒有資料」**（與盤點文件自己的「跑了也沒可發布活動」一致，只是當時沒追問「那資料在哪」）。**資料在場館 / 售票 / 主辦端且全部公開無封鎖**（Kai Tak / SoFi / Gillette / Galaxy Macau / Tokyo Dome —— B-1a 的 17 個 citation 全從這類來源取得，5/5 精準、$0.049/event）；**結構性理由：經紀公司沒動機即時更新官網，場館與售票商必須更新因為要賣票**。⚠️ **決議：Scrapling 線（PR #185 工作單）不投** —— 它解「進不去」，我們的問題是「裡面沒有」，期望報酬近零；工作單保留為歷史紀錄，不執行。⚠️ **真瀏覽器不可作 runtime**：不可自動化（需 Owner 桌機開著 + agent 在場）、不可部署（Vercel cron 無法驅動桌面瀏覽器）→ 它是**探測工具**與一次性補資料工具，不是每日管道。**下一步建議 = B-6 discovery 探測票**：把已驗證的 B-direct 設定從「求證既有候選」轉為「對零候選藝人主動發現行程」；⚠️ **但求證 ≠ 發現，不可假設**（求證有標準答案可比對已證明 5/5；發現無候選可對照、假陽性更危險、且「查無」與「真的沒有」難分）→ **必須先探測再實作**。 ✅ **分支堆疊問題已解除（2026-07-15）**：本線分支曾**層層疊上去**（`main ← b463ac8 scrapling(#185) ← 8660dc2 盤點文件 ← 694a886 B-0(#187) ← 623789f B-0c(#188) ← 31c3e8e 文件 ← 33d16ec B-1 報告`），三個 PR 從同一條鏈的不同點對 main 開。**已於 2026-07-15 用 merge commit `ab4bc04` 一次 merge #188 解決**：因 diff 是累積的（#188 含 #185/#187 全部檔案），`b463ac8` / `694a886` 隨之成為 main 祖先，**#185 / #187 自動標為 MERGED**，#186 的 `cac8ea5` 完好未失。**教訓：真正的風險不是 merge 順序，是 squash** —— 本 repo 混用 squash（#182/#183/#184）與 merge commit（#186），若當時 squash-merge #185，`b463ac8` 會被改寫成新 commit，#187/#188 立刻衝突，順序對了也沒用。**日後一律 `git fetch origin main && git checkout -b <new> origin/main` 從乾淨 main 開，不要再疊。** ⟶ 以下為前次 checkpoint： **2026-07-14 Checkpoint（資料缺口盤點 → 補洞 → 聚合源自動求證線）**：角色分工改為 PM(Claude 開票/審核/文件)/實作(Codex)/Owner，見 CLAUDE.md 新增段。**PR #186 merged**（開放聚合源批量核准，移除前端+後端兩道 needsOriginalSource 閘門，草稿 trust_level=pending 安全邊界不變）。**資料盤點結論**：已發布 156 場集中在 20 藝人，72/92 active idol 零活動；72 黑洞中僅 9 個有候選（ENHYPEN 17待審 / Jay Park 17待審 / CORTIS 9 / (G)I-DLE 5草稿 / DAY6 3 / ILLIT 2 / JENNIE 1草稿 / aespa / NMIXX），其餘 63 個零候選＝真來源缺口。**核心矛盾**：99 草稿中 94 筆是 community 聚合源，受 `inferTrustLevelFromSource` 閘門擋住（community→pending→不可發布，`CLAUDE.md` 前台只顯示 official/media 的守門，設計正確非 bug）；已發布 156 場中僅 27 筆聚合源靠人工升級（已達人力極限，Owner 否決繼續人工）。**Owner 決策 D1＝選項 B「自動求證管道」**：對聚合事件自動搜尋官方/售票/媒體佐證→命中掛真實 source（trust 自然升）→admin 一鍵確認發布；provider 首選 **Gemini（Google Search grounding）**，需 GEMINI_API_KEY（走 secrets 規則）。**三份新文件**：`DATA_GAP_AUDIT_WORK_ORDER.md`（盤點+SQL）/ `DATA_BACKFILL_PLAN.md`（L1-L3+接受空白四類槓桿、D1 決策）/ `AGGREGATOR_EVENT_VERIFICATION_WORK_ORDER.md`（B 選項高風險工作單，含 Gemini/§6 假陽性把關/B-0~B-5 PR 拆分）。**短期免費補洞（尚未做）**：Jay Park(17)+DAY6(3) 官方源待審→核准→發布 = +2 藝人乾淨資料。**開群結論**：巡演垂直群（聚焦 20 個有料團）可行、全藝人大群不可行，不必等 B 完成。**下一步（新流程：PM 開票→Codex 實作→PM review→Owner merge，無 GPT audit）**：(1) PM 開 **B-0 Gemini grounding 探測票**交 Codex（先測 ENHYPEN/BTS World Tour 幾筆的命中率+假陽性率+成本，比照 P1-A/B「先探測再實作」，**不先寫 runtime**）；(2) 之後 B-1 migration→B-2 求證 runtime→B-3 UI，逐張開票。**不要做**：人工逐筆補聚合源（Owner 否決）/ Drift Diff v2 / Weverse runtime。 ⟶ 以下為前次快照： **2026-05-25 狀態（本週末交接快照）**。P1-B generic_webpage crawler 全線完成（B1～B8，migrations 049-054）。Drift Diff v1 完成（migration 058，JYP fetcher 接入，admin 詳情頁差異對比 UI）。個人藝人 notice URL 研究完成（Verdict A：Jay Park / JISOO / DEAN / PLAVE）。**Migration 056 已執行 + Preview 驗收完成（2026-05-25）**：JISOO（high/10 events 已 commit 8）+ PLAVE（medium/10 events 已 commit 9 + 1 inserted）成功；Jay Park & Dean 落地頁 pageRelevance=none → migration 059 curl 探測子路徑（`/notice` + `/live`）**已寫 SQL（PR #182），等 GPT audit → merge → Supabase 手動跑 → Preview 重驗**。RSS Feed 探測完成（PR #184 使用者已確認內容 OK，等正式 merge）：11 個 domain 全部探測；結論否決 wordpress_rss runtime 開發（Verdict A 的 WAKEONE + kpopofficial 不走 Claude，8 個真正燒 Claude token 的 generic_webpage 來源全部 Verdict C/B）；7 個來源已加入「不要重複嘗試」清單。Weverse / Weverse Shop 雙輪公開頁探測完成（Verdict C/D，已加入「不要重複嘗試」清單）。Migrations 050–058 全部已執行。PRs #170–#183 全部已 merged。**待處理**：(1) merge PR #182 → 跑 migration 059 SQL → Preview Jay Park + Dean → Commit；(2) merge PR #184（RSS probe report，內容已 OK）；(3) 決定下一資料來源方向（Ticketmaster Discovery API WO / Search Discovery v2 / YouTube P2-B / 第二批 solo artist seed）；(4) Drift Diff v2（接其他 6 個 crawler）、Weverse runtime — **明確暫緩，不要自行開工作單**。kpopofficial 健康運作中。91 個 active idol。 |
| 輔助參考 | `ADMIN_ROADMAP.md`（後台分階段開發路線）、`AI_PIPELINE_PLAN.md`（爬蟲架構設計文件）、`SOURCE_INVENTORY_A.md`（官方來源盤點研究附件；不是流程索引，流程仍以本檔為準）、`MAINSTREAM_ARTIST_SEED_WORK_ORDER.md`（主流漏網藝人 seed 工作單；只做規劃）、`CHINESE_GENERATION_WORK_ORDER.md`（繁中顯示文案生成工作單；只做規劃）、`NOTIFICATION_SYSTEM_WORK_ORDER.md`（通知系統短版工作單；只做規劃）、`NOTIFICATION_DATA_MODEL_WORK_ORDER.md`（通知資料模型工作單 + migration 草案 SQL；只做規劃）、`CRAWLER_WORK_ORDER_STARSHIP.md`（Starship Entertainment Phase A probe：Verdict Google Discovery；IVE / MONSTA X / CRAVITY / KiiiKiii 目前走 Google Discovery 路徑等待決策）、`STREAMING_VIDEO_SOURCE_WORK_ORDER.md`（P2 Streaming / Video Source Inventory 工作單，PR #144 merged）、`YOUTUBE_CHANNEL_CRAWLER_WORK_ORDER.md`（P2-A YouTube Official Channel Crawler 工作單，PR #145 merged）、`SEARCH_DISCOVERY_WORK_ORDER.md`（P1 Search Discovery Provider 工作單，PR #149 merged）、`GOOGLE_PSE_PROBE_REPORT.md`（P1-A Google PSE 探測報告，Verdict Deferred）、`CLAUDE_WEBPAGE_DISCOVERY_WORK_ORDER.md`（P1-B Claude Webpage Discovery 工作單，PR #152 merged；P1-B1 runtime 已 ship PR #153）、`CLAUDE_WEBPAGE_IDOLMATCHER_WORK_ORDER.md`（P1-B8 generic_webpage 雙改進工作單：idolMatcher 整合 + MAX_CANDIDATES_PER_COMMIT 可配置，PR #167 merged + #168 擴充）、**`DATA_GAP_AUDIT_WORK_ORDER.md`（2026-07-14 資料缺口盤點：72 黑洞、20 涵蓋藝人、含 SQL-1~7 盤點查詢與實測數字）**、**`DATA_BACKFILL_PLAN.md`（補洞計畫：L1 審核發布/L2 啟用未跑源/L3 Scrapling/接受空白 四類槓桿 + D1 聚合源發布決策，Owner 選選項 B）**、**`AGGREGATOR_EVENT_VERIFICATION_WORK_ORDER.md`（聚合源自動求證工作單，B 選項，高風險；§7 PR 拆分 B-0→B-1 spike→B-1b migration→B-2 runtime→B-3 UI；**provider 已由 Gemini 改為 Claude web search**，見 B-0/B-0c 報告）**、**`AGGREGATOR_VERIFICATION_B0_PROBE_REPORT.md`（B-0 Gemini 探測，Verdict **BLOCKED**，需 billing/quota，Owner 已否決此路，PR #187 merged）**、**`AGGREGATOR_VERIFICATION_B0C_PROBE_REPORT.md`（B-0c Claude web search 探測，命中 5/5、假陽性 0/5、$0.93、**Verdict 需調整**：citations 0/5 為唯一卡點，PR #188 merged）**、**`AGGREGATOR_VERIFICATION_B1_SPIKE_REPORT.md`（B-1 citation binding spike：A dynamic citations 0/5 / **B `allowed_callers:["direct"]` 5/5** / C basic 5/5，假設成立；**Verdict 可進 B-1b，但 runtime 僅採 B-direct contract + 強制 deterministic re-check / fail closed**；⚠️ 其「假陽性 0/5」為空指標，樣本全是真候選 → 須先過 B-1a negative control）**|
...
| 102 | P1-B Claude Webpage Discovery 工作單 | `CLAUDE_WEBPAGE_DISCOVERY_WORK_ORDER.md`：規劃 `generic_webpage` parser — 管理員提供 URL 後，fetch 公開頁 HTML、cheerio 清理、送 Claude Haiku 解析 K-pop 活動、P1-B1 preview-only / P1-B2 commit 兩階段、全部進 `event_candidates`（review_status='pending'）人工審核。source_hash 優先 url+title+date+type，fallback url+title+snippet[:100]。不接 fan-out / sync-all。migration 049 seed 1–3 個測試 URL。 | ✅ PR #152 merged |
| 103 | P1-B1 generic_webpage preview-only runtime | `src/lib/crawlers/genericWebpage.ts`（fetchPublicHtml + cleanHtmlToText + parseWebpageWithClaude + assertUrlIsPublic SSRF guard）+ `runGenericWebpageFetcher.ts`（preview-only orchestrator，**完全不寫 event_candidates**）+ `POST /api/admin/crawlers/generic-webpage/run`（admin-only，`mode='preview'` 唯一接受，`commit` 回 400 留給 P1-B2）+ `RunSourceButton` 新增 case + preview-aware display + `runActiveCrawlerSources` dispatch 雙重 skip（cron + sync-all guard）+ `supabase/migrations/049_seed_generic_webpage_test_sources.sql`（3 筆 wiring probe，全 is_active=false）。SSRF guard 覆蓋：scheme（只 http/https）、port（只 80/443）、hostname literal blocklist（localhost / *.localhost / *.local）、字面 IP 與 DNS 解析後 IP 比對（0/8, 10/8, 127/8, 169.254/16, 172.16-31/12, 192.168/16, ::1, fc00::/7, fe80::/10）；拒絕時不進 fetch、不呼叫 Claude。fetch 邊界：timeout 10s、maxHtmlBytes 500KB、403/429/Cloudflare fail gracefully。Claude prompt：rawTitle/rawSnippet 必填、pageRelevance=none 強制 events=[]、maxEventsPerPage=10 按 confidence 截斷。PR #155：`page.tsx` 手動執行按鈕條件改為 `isActive \|\| parserType === 'generic_webpage'`（其他 parser 仍需 is_active=true）。**P1-B1 wiring probe 驗收完成（2026-05-24）**：3 source 全部無 crash，event_candidates 零寫入確認。 | ✅ PR #153 + #155 merged，migration 049 已執行，wiring probe ✅ |
| 104 | P1-B2 generic_webpage commit runtime 工作單 | `GENERIC_WEBPAGE_COMMIT_WORK_ORDER.md`：設計 `mode='commit'` 寫入路徑；PreviewEventType → event_type 對應表；dateHint 解析策略（4 步驟，失敗留 null）；source_hash 計算（primary: url+title+date+type，fallback: url+title+snippet[:100]）；raw_data 結構含 dedupe_basis；pageRelevance=none 守門；RunSourceButton 新增獨立 Commit 按鈕（amber 色）；CommitResult type；無需新增 migration。 | ✅ PR #157 merged（GPT audit 通過）|
| 105 | P1-B2 generic_webpage commit runtime 實作 | `runGenericWebpageFetcher.ts` 新增 `runGenericWebpageCommit`（呼叫 preview 內部、pageRelevance=none 守門、confidence gate 0.65、type mapping、maxCandidatesPerCommit=3 整批拒寫、SELECT 既有 hash 去重、per-row INSERT + 23505 = deduped、updateRunStatus with summary）+ `CommitSummary` / `CommitResult` types + `parseDateHint` / `computeSourceHash` / `buildCandidatePayload` helpers；`route.ts` 改為 5 guards（admin auth / batch ban / sourceKey / mode whitelist / confirmCommit=true）+ `PreviewResponse` / `CommitResponse` / `ErrorResponse` discriminated union；`RunSourceButton.tsx` 新增獨立 Commit 按鈕（amber 色，`Database` icon）+ `commitGateOpen`（需 `result.mode==='preview' && result.ok && errors.length===0`）+ independent `committing` state + `window.confirm` 二次確認 + 完整 commitSummary display + warnings list。**驗收完成（2026-05-24）**：Preview 行為不變；Commit 只有成功 Preview 後可按；window.confirm cancel → 無請求；pageRelevance=none → inserted=0；重複 Commit → deduped=N；`raw_data.provider='generic_webpage'` + `dedupe_basis` 正確。 | ✅ PR #158 merged（驗收完成 2026-05-24）|
| 106 | P1-B3 hit-quality test | migration 050 seed (G)I-DLE Cube News source；手動觸發 Preview → 實際 K-pop 活動候選命中驗收；確認 generic_webpage parser 對 real label 頁面的品質 | ✅ PR #160 merged，migration 050 已執行（2026-05-24）|
| 107 | P1-B4 Dynamicduo / Amoeba Culture | migration 051 seed `generic-dynamicduo-amoeba`（source_url=`/artists/67`）；Preview 驗收通過 | ✅ PR #161 merged，migration 051 已執行（2026-05-24）|
| 108 | P1-B5 YG notice + P1-B6 N.Flying JP | migration 052+053 seed YG notice + N.Flying JP sources；Preview 驗收通過 | ✅ PR #163 merged，migrations 052+053 已執行（2026-05-24）|
| 109 | P1-B7 kpopconcerts.com aggregator | migration 054 seed `generic-kpopconcerts`（idol_id=NULL，聚合多藝人頁）；Preview 驗收通過（inserted=1）；aggregator guard 阻止 kpopofficial-style 重複來源 | ✅ PR #165 merged，migration 054 已執行（2026-05-24）|
| 110 | P1-B8 generic_webpage 雙改進實作 | `runGenericWebpageFetcher.ts`：(A) 整合 idolMatcher（`loadIdolMatcherIndex()` + `resolveDetectedIdol()`，idol_id=null 的來源自動比對）+ `IdolMatchedVia` / `IdolMatchedEvent` types + preview 結果加 `idolMatchResults` 欄位；(B) `MAX_CANDIDATES_PER_COMMIT` 3→10 + per-source config 可覆蓋（`DEFAULT_MAX_CANDIDATES_PER_COMMIT=10`、`MAX_CANDIDATES_PER_COMMIT_HARD_CAP=50`、`resolveMaxCandidates(source)` resolver）；`buildCandidatePayload` 接受 `detectedIdolId` + `idolMatchedVia`；只影響手動 Preview/Commit，不影響 cron / 其他 parser | ✅ 待 merge（pending PRs #170-172）|
| 111 | 修 BTS / (G)I-DLE 發布 crash（AGGREGATOR_PATTERNS）| `inferTrustLevelFromSource` 的 `AGGREGATOR_PATTERNS = ['kpopofficial','kpop official','concerts']` 擋下 BTS / (G)I-DLE publish，throw Error 在 Server Action 未被 ErrorBoundary 接住 → 通用錯誤頁；需 admin 手動補官方來源到 event_sources，或修 pattern 邏輯 | ✅ 修正合併（pending PRs #170-172）|
| 112 | 個人藝人 notice URL 研究工作單 | `SOLO_ARTIST_NOTICE_URL_RESEARCH_WORK_ORDER.md`：規劃 ~20 位個人藝人（Jay Park / JENNIE / LISA 等）的公開 notice URL 研究策略；curl 探測計畫；Verdict A/B/C 分類；only 工作單，不寫 crawler | ✅ PR #173 merged |
| 113 | 個人藝人 notice URL 探測報告 | `SOLO_ARTIST_NOTICE_URL_PROBE_REPORT.md`：curl 實測 ~20 位個人藝人；**4 個 Verdict A**（Jay Park → jaypark.me、JISOO → blissoo.com、DEAN → youwillknovv.com、PLAVE → vlast.co.kr）；其餘 Verdict C（JENNIE/LISA SPA / G-DRAGON Weverse / 其他 SPA 或無官網）| ✅ PR #174 merged |
| 114 | Seed 4 Verdict A solo 藝人來源（migration 056）| `056_seed_generic_webpage_solo_artist_batch1.sql`：seed generic-jay-park-jaypark / generic-jisoo-blissoo / generic-dean-youwillknovv / generic-plave-vlast；全部 is_active=false，待 Preview 驗收後再啟用 | ✅ migration 056 已執行 + Preview 驗收完成（2026-05-25；JISOO high/10 events + 8 已 commit；PLAVE medium/10 events + 9 已 commit；Jay Park & Dean pageRelevance=none → 子路徑修復見 #121）|
| 115 | Cleanup generic_webpage 測試來源（migration 057）| `057_cleanup_generic_webpage_test_sources.sql`：DELETE 3 筆 wiring probe sources（example-com / wikipedia-blackpink / wikipedia-iu）；它們是 P1-B1 暫時測試用，已完成使命；PR #175/176 | ✅ PR merged，migration 057 已執行（2026-05-25）|
| 116 | 批量 resolve needs_recheck + tab 範圍 checkbox | `CandidatesClient.tsx`：`setTab` 清除 selection；`visibleRecheck` / `allRecheckSelected` / `toggleAllRecheck()` / `bulkResolveRecheck()`；`isCheckable` tab 範圍限制（pending tab → isPending，recheck tab → needsRecheck&&!isPending）；toolbar 依 tab 切換文案與動作。新 `POST /api/admin/event-candidates/bulk-resolve-recheck`：per-row 同步中文欄位 → 清 needs_recheck；uses cookie server client + admin auth gate | ✅ PR #177，待 merge |
| 117 | Drift Diff 工作單 | `DRIFT_DIFF_WORK_ORDER.md`：規劃來源內容變更後後台顯示新舊差異；方案 A（5 個 latest_* 欄位）vs 方案 B（history 表）；v1 採方案 A，只接 JYP schedule fetcher；v2 接其餘 6 個 crawler；admin 詳情頁側邊對比；10 條設計限制 | ✅ PR #178 merged |
| 118 | Drift Diff v1 實作（JYP fetcher + admin UI + migration 058）| `runJypScheduleFetcher.ts`：drift branch UPDATE payload 從 3 欄位擴為 8 欄位（+latest_raw_title / latest_raw_content / latest_detected_date / latest_source_url / latest_detected_at）；原始 raw_* 欄位不覆寫。`src/app/admin/event-candidates/[id]/page.tsx`：CandidateDetail 加 5 個 latest 欄位；DiffField 元件；「內容變更紀錄」section（側邊對比 + emerald 高亮 + 警告 banner）。migration 058：ADD COLUMN 5 個 latest_* + GRANT UPDATE TO service_role | ✅ PR #179 merged，migration 058 已執行（2026-05-25）|
...
| 123 | RSS Feed 探測報告 | `RSS_FEED_PROBE_REPORT.md`：11 個 domain curl 實測。**結論：否決 wordpress_rss runtime 開發**。Verdict A：WAKEONE (`/news/feed/` 12 items + `/notice/feed/` 12 items) + kpopofficial (`/feed/` 15 items)；但這兩個來源**目前都不走 generic_webpage / 不耗 Claude token**（各用獨立 cheerio parser），原始動機「降低 Claude 成本」無法兌現。所有真正用 generic_webpage 的 8 個來源（BLISSOO / VLAST / Cube / Amoeba / Jay Park / Dean / N.Flying JP / YG）全部 Verdict C 或 B（無可用 feed）。建議 7 個來源加入「不要重複嘗試」清單，把資源轉向 Ticketmaster / Search Discovery / YouTube 擴張 | ✅ PR #184 使用者已確認 OK（2026-05-25）；等正式 merge |
| 124 | Scrapling 公開頁探測工作單 | `SCRAPLING_PROBE_WORK_ORDER.md`：評估 Scrapling Python 函式庫（StealthyFetcher / CamoufoxFetcher）能否突破 curl 失敗的反爬機制，抓取 Starship / Cube / THEBLACKLABEL / P NATION / RBW / WM / FNC / EDAM 等來源的公開 notice / artist 頁。測試方法：本機 Python probe script，curl 對比，HTML 大小 + 關鍵字命中驗收，Verdict A/B/C/D；≥3 個 Verdict A → 開 Stealthy runtime 工作單；≥3 個 Verdict B → 開 Playwright fetcher 工作單；否則加入「不要重複嘗試」清單。不寫 runtime / 不改 schema / 不 seed / 不接 production | 🔲 本 PR；等 GPT audit + merge |

---

## 不要重複嘗試（已探測確認無法使用）

以下來源已經過 curl 探測或工程評估，確認在無登入 / 無特殊 auth 條件下**無法用於爬蟲**。後續不要再開工作單或 runtime 嘗試，除非技術條件有根本性變化（如官方 API 開放）。

| 來源 | 判定 | 原因 |
|---|---|---|
| weverse.io（所有藝人/路徑）| ❌ Verdict C — SPA shell | 全域 200 + 5478 bytes React SPA；`<div id="root"></div>` 無內容；需 auth header；PR #131 + #181 雙輪探測確認 |
| shop.weverse.io（藝人/產品頁）| ❌ Verdict D — 無可用路由 | `_buildManifest.js` 確認 SSR 只有 /home；無 per-artist / per-product SSR 路由；PR #181 |
| hybe.com / ibighit.com / pledis.com / sourcemusic.com / beliftlab.com / adorent.com / kozent.com | ❌ 結構性封鎖 | 503/403/404/SPA；HYBE 全系列；P1-B8 curl 批次確認 |
| instagram.com（藝人頁）| ❌ SPA | 全 SPA，無 server-rendered 內容 |
| ticketing：Interpark / YES24 / Melon / Ticketmaster / Pia / e+ / AXS / Songkick / concertful / bandsintown | ❌ SPA 或反爬 | P1-B8 批次確認 |
| soompi.com / allkpop.com | ❌ 品質差 | soompi 404；allkpop 雜訊高，不適合爬蟲 |
...
| 049 | Seed 3 筆 generic_webpage wiring probe（generic-test-baseline-example-com / generic-test-wikipedia-blackpink / generic-test-wikipedia-iu），全部 `is_active = false`、`parser_type = 'generic_webpage'`、`source_type = 'unknown'`（修正自 'other'，enum 無此值）、`idol_id = NULL`；ON CONFLICT (source_key) DO UPDATE 不覆蓋 is_active；config 含 test_category（baseline_non_kpop / kpop_group_wiring_probe / kpop_solo_wiring_probe）；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24；source_type 修正為 'unknown'）|
| 050 | Seed (G)I-DLE Cube News generic_webpage source（P1-B3 hit-quality test）：`generic-gidle-cube-news`，idol_id=gidle，source_url=`https://www.cube-ent.com/en/news`，is_active=false，parser_type=generic_webpage；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24）|
| 051 | Seed Dynamicduo / Amoeba Culture generic_webpage source（P1-B4）：`generic-dynamicduo-amoeba`，idol_id=dynamicduo，source_url=`https://www.amoebaculture.com/artists/67`（正確藝人頁），is_active=false；ON CONFLICT DO UPDATE；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24）|
| 052 | Seed YG notice generic_webpage source（P1-B5）：`generic-blackpink-yg-notice`，idol_id=blackpink，source_url=YG notice URL，is_active=false；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24）|
| 053 | Seed N.Flying JP generic_webpage source（P1-B6）：`generic-nflying-jp`，idol_id=nflying，source_url=N.Flying JP 官網活動頁，is_active=false；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24）|
| 054 | Seed kpopconcerts.com aggregator generic_webpage source（P1-B7）：`generic-kpopconcerts`，idol_id=NULL，source_url=`https://kpopconcerts.com`，is_active=false；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-24）|
| 055 | Fix Dynamicduo source URL：UPDATE `generic-dynamicduo-amoeba` source_url 回 `/artists/67`（修正被手動改成 `/news` 的問題）；idempotent WHERE clause；不改 schema / GRANT / RLS | ✅ DB 已修正（source_url = /artists/67；migration UPDATE 為 no-op，DB state 正確）|
| 056 | Seed 4 個 solo 藝人 notice URL generic_webpage sources（Verdict A）：generic-jay-park-jaypark / generic-jisoo-blissoo / generic-dean-youwillknovv / generic-plave-vlast；全部 is_active=false；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-25；4 筆全部確認：parser_type=generic_webpage / source_type=official_website / is_active=false / has_idol=true）|
| 057 | Cleanup 3 個 generic_webpage 測試 wiring probe sources：DELETE generic-test-baseline-example-com / generic-test-wikipedia-blackpink / generic-test-wikipedia-iu；不改 schema / GRANT / RLS | ✅ 已執行（2026-05-25；3 筆全部刪除確認）|
| 058 | Drift Diff v1：ADD COLUMN 5 個 latest_* snapshot 欄位到 event_candidates（latest_raw_title / latest_raw_content / latest_detected_date / latest_source_url / latest_detected_at）+ GRANT UPDATE (5 cols) TO service_role；v1 只接 JYP schedule fetcher；不改 RLS / 既有欄位 / 其他 crawler | ✅ 已執行（2026-05-25；ADD COLUMN + GRANT 均確認）|
| 059 | Fix solo artist subpath URLs：UPDATE 2 rows in crawler_sources — generic-jay-park-jaypark → `https://www.jaypark.com/notice`（Next.js SSG with inline __NEXT_DATA__ containing real tour/event posts），generic-dean-youwillknovv → `https://www.youwillknovv.com/live`（imweb 平台，含 3 個未來日期）；jsonb_set 同步更新 config.note；不改 schema / GRANT / RLS / parser | 🔲 待執行（migration SQL 已寫，需至 Supabase SQL Editor 手動跑）|
...
| `/api/admin/crawlers/generic-webpage/run` | POST | P1-B1 generic_webpage preview-only（admin guard；body: {sourceKey, mode:'preview'}；mode!='preview' 一律 400；不寫 event_candidates；不接 cron / sync-all）|
...
| `/api/admin/event-candidates/bulk-resolve-recheck` | POST | 批量清除 needs_recheck flag（PR #177）；per-row：若 approved+approved_event_id 則先同步中文欄位到 event，成功才清旗；uses cookie server client + getCurrentAdmin() |
