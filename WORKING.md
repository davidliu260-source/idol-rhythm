...
| 104 | P1-B2 generic_webpage commit runtime 工作單 | `GENERIC_WEBPAGE_COMMIT_WORK_ORDER.md`：設計 `mode='commit'` 寫入路徑；PreviewEventType → event_type 對應表；dateHint 解析策略（4 步驟，失敗留 null）；source_hash 計算（primary: url+title+date+type，fallback: url+title+snippet[:100]）；raw_data 結構含 dedupe_basis；pageRelevance=none 守門；RunSourceButton 新增獨立 Commit 按鈕（amber 色）；CommitResult type；無需新增 migration。 | 🔒 待 GPT 工作單審核 |
