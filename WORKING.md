...
| 項目 | 值 |
|---|---|
| 目前階段 | **2026-05-25 狀態（更新）**。P1-B generic_webpage crawler 全線完成（B1～B8，migrations 049-054）。Drift Diff v1 完成（migration 058，JYP fetcher 接入，admin 詳情頁差異對比 UI）。個人藝人 notice URL 研究完成（Verdict A：Jay Park / JISOO / DEAN / PLAVE）。**Migration 056 已執行 + Preview 驗收完成（2026-05-25）**：JISOO（high/10 events 已 commit 8）+ PLAVE（medium/10 events 已 commit 9 + 1 inserted）成功；Jay Park & Dean 落地頁 pageRelevance=none → migration 059 curl 探測子路徑（`/notice` + `/live`）待執行。Weverse / Weverse Shop 雙輪公開頁探測完成（Verdict C/D，已加入「不要重複嘗試」清單）。Migrations 050–058 全部已執行。PRs #170–#181 全部已 merged（2026-05-25）。**待處理**：(1) migration 059 待至 Supabase SQL Editor 執行，並重做 Jay Park / Dean Preview 驗收；(2) Drift Diff v2（接其他 6 個 crawler）、Weverse runtime — **明確暫緩，不要自行開工作單**。kpopofficial 健康運作中。91 個 active idol。 |
