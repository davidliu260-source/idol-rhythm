...
| 044 | Seed WAKEONE notice crawler_sources：zerobaseone-wakeone-notice / kep1er-wakeone-notice / izna-wakeone-notice；全指向 `https://wake-one.com/notice/`，`parser_type = 'wakeone_notice'`，`source_type = 'official_website'`；ON CONFLICT (source_key) DO UPDATE；不改 schema / GRANT / RLS | 🔲 待執行（PR merged 後在 Supabase SQL Editor 執行）|
