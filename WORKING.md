...
| 106 | P1-B3 generic_webpage 實戰 hit-quality 驗收 | migration 050（`050_seed_generic_webpage_aespa_smtown_notice.sql`）：seed 1 筆真實 `generic_webpage` source — `aespa — SMTOWN Notice`（`https://www.smtown.com/notice`，`source_key=generic-sf9-fnc-notices`，`idol_id=aespa`，`is_active=false`）。discovery process：(1) cube.co.kr/en/news → 404（cube.co.kr 是不相關印刷公司）；(2) fncent.com/Notices → pageRelevance=none（路徑為行政公告，無活動內容）；(3) fncent.com → pageRelevance=none（NOTICE 區塊為 AJAX 動態載入，fetcher 看不到）；(4) smtown.com/notice → server-rendered WordPress，命中。**驗收完成（2026-05-24）**：Preview pageRelevance=medium、4 events、0 errors；Commit inserted=3、deduped=0；3 筆候選 review_status=pending、provider=generic_webpage、dedupe_basis 正確；events 正式表零寫入；gate 在 Commit 後正確關閉。**教訓**：AJAX-loaded 頁面對 generic_webpage parser 不可用；需找 server-rendered 頁面（WordPress / 靜態 HTML）。 | ✅ PR #160 merged，migration 050 已執行（2026-05-24）|
