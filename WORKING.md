...
| 84 | N6 event_reminder 派送 Cron 實作 | `src/app/api/cron/dispatch-reminders/route.ts` + `vercel.json`：CRON_SECRET auth → service_role query → day_before/week_before 日期視窗過濾 → upsert notifications（ignoreDuplicates）→ is_sent 標記；upsert 或 is_sent 更新失敗皆回 500；排程 `30 1 * * *`（09:30 Taipei）| ⏳ PR #119 pending merge（Vercel CI 驗證中）|
