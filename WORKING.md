...
| 049 | Seed 3 筆 generic_webpage wiring probe（generic-test-baseline-example-com / generic-test-wikipedia-blackpink / generic-test-wikipedia-iu），全部 `is_active = false`、`parser_type = 'generic_webpage'`、`source_type = 'other'`、`idol_id = NULL`；ON CONFLICT (source_key) DO UPDATE 不覆蓋 is_active；config 含 test_category（baseline_non_kpop / kpop_group_wiring_probe / kpop_solo_wiring_probe）；不改 schema / GRANT / RLS | 🔲 待執行 |
