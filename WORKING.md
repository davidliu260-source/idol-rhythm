...
| 047 | Seed Jo Yuri WAKEONE notice crawler_source：jo-yuri-wakeone-notice；1 筆；source_url = `https://wake-one.com/notice/`；parser_type = `wakeone_notice`；Phase A probe 確認 Jo Yuri 已加入 WAKEONE（wake-one.com/artists/joyuri/ 確認）+ notice board server-rendered WordPress + fetcher per-idol 匹配安全；ON CONFLICT (source_key) DO UPDATE；不改 parser / schema / RLS | 🔲 待使用者在 Supabase SQL Editor 執行 |
