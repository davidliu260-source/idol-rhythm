...
| 95 | 後台用戶統計 runtime 實作 | `src/lib/supabase/analyticsStats.ts`（server-only，service_role block try/catch + server client block Promise.allSettled；auth.users 用 `.schema('auth').from('users')`；user-scoped 四表全站 aggregate 也走 service_role；admin_users 走 service_role；events / event_candidates / crawler_sources 走 server client；candidates / sources by type 用 JS groupBy；全部 Promise.allSettled 並行）+ `src/app/admin/analytics/page.tsx`（force-dynamic / getCurrentAdmin() redirect / service_role unavailable banner / 7 個 section / StatCard / MetricRow / ServiceRoleNote 純 Server Component）+ admin dashboard 加「用戶統計」Link + `serviceClient.ts` allowed callers 更新。不新增 migration / 不改 schema / 不改 RLS / 不寫 API route / 不改前台 UI / 不改 crawler | 🔒 PR opened，等 GPT audit |
