...
| 046 | Seed 額外 SMTOWN notice crawler_sources：nct-127 / nct-dream / nct-wish / shinee / super-junior / taeyeon / tvxq / hearts2hearts；8 筆；同 045 pattern；NCT unit matching 安全確認（unit-specific alt_names + NCT_UNIT_NAMES_LOWER guard）；ON CONFLICT (source_key) DO UPDATE；不改 parser / schema / RLS | 🔲 待使用者在 Supabase SQL Editor 執行 |
