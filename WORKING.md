...
| 96 | 快閃店搜尋與呈現工作單 | `POPUP_STORE_SEARCH_WORK_ORDER.md`：確認 migration 041 三個 enum 值（popup_store / exhibition / brand_event）、TypeScript types、EventTypeBadge、EVENT_SUBTYPE_LABELS 全部已就緒，無需 migration。識別兩個前台缺口：A — `getSearchHaystack` 不含 subtype label，搜「快閃店」不命中；B — `/schedule` filter 三種 subtype 捆綁在「品牌快閃」一個 chip。規劃搜尋 haystack 補 subtype label + 英文關鍵字別名（popup / pop-up / exhibition / brand event），以及 filter chip 拆分方案（方案 X：拆出三個獨立 chip；方案 Y：只修搜尋保持 chip 不變，GPT 擇一）。評估 /schedule / /events/[id] / 首頁 / favorites 影響範圍；詳列實作前必查 SQL；白話驗收 12 項。只工作單，不改 code / schema / migration / admin / crawler | 🔒 PR opened，等 GPT audit |
